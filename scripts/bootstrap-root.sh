...
sudo --preserve-env=DISCORD_BOT_TOKEN,DISCORD_TARGET,DISCORD_HUMAN_ID,DISCORD_USER_ID,DISCORD_HUMAN,ANTHROPIC_API_KEY,FRONTEND_ENABLED,FRONTEND_ALLOWED_IP,EXEC_APPROVAL_MODE,OPERATOR_BRIDGE_PORT,SYNTELLA_EXEC_TIMEOUT_SECONDS,SYNTELLA_EXEC_MAX_OUTPUT_BYTES -u "$OPENCLAW_USER" -H bash -lc "cd '$REPO_DIR' && bash scripts/bootstrap-openclaw.sh"
